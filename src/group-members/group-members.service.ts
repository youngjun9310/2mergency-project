...
export class GroupMembersService {
  ...
  async inviteUserToGroup(groupId: number, userId: number, email: string) {
    ...
    if (existingInvite) {
      if (existingInvite.isVailed) {
        throw new BadRequestException(`${userToInvite.userId}유저는 이미 ${groupId}그룹의 멤버입니다.`);
      } else {
        throw new BadRequestException(`${userToInvite.userId}유저는 이미 ${groupId}그룹의 초대가 발송되었습니다.`);
      }
    }
  ...
  async acceptInvitation(groupId: number, userId: number, email: string): Promise<any> {
    ...
    if (!user || user.userId !== userId) {
      throw new NotFoundException(`${user.userId}유저가가 존재하지 않거나 일치하지 않습니다.`);
    }
  ...
  async getAllGroupMembers(
    ...
    if (!members.length) {
      throw new NotFoundException(`그룹 ID ${groupId}에 해당하는 멤버가 없습니다.`);
    }
