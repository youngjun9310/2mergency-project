...
export class GroupMembersController {
  ...
  async isGroupMember(
    ...
    if (!memberDetails) {
      throw new NotFoundException(`해당 사용자${userId} 또는 그룹${groupId}이 데이터 베이스에 존재하지 않습니다.`);
  ...
  async findByUserAndGroup(@Param('userId') userId: number, @Param('groupId') groupId: number): Promise<GroupMembers> {
    const groupMember = await this.groupMembersService.findByUserAndGroup(userId, groupId);
    if (!groupMember) {
